/**
 * Cradlepoint Amazon Storefront - Main TypeScript
 * Dropdown nav + button ripple + product card hover + enhanced scroll selectors
 */
...
(function (): void {
  ...
  function initProductToggles(): void {
    const toggleConfigs: readonly ToggleConfig[] = [
      { buttonId: 'wwan-routers-see-more', containerId: 'wwan-routers-products' },
      { buttonId: 'wwan-adapters-see-more', containerId: 'wwan-adapters-products' }
    ];
    ...
    toggleConfigs.forEach(({ buttonId, containerId }: ToggleConfig): void => {
      ...
      const handleToggle = (): void => {
        ...
        if (isExpanded) {
          ...
          const parent: HTMLElement | null = extraProducts.length
            ? (extraProducts[0] as HTMLElement).parentElement
            : null;
          ...
          const totalCards: number = extraProducts.length;
  ...
  function initScrollAnimations(): void {
    const animatedElements: NodeListOf<Element> = document.querySelectorAll(
      '.story-card, .category-tile, .product-card:not(.product-card--extra), .stat-item, .testimonial, .peace-of-mind, .contact-cta, .from-manufacturer__item, .cert-badge, .category-banner, .btn--modern, .btn--gradient'
    );
  ...
  function initCategoryTiles(): void {
    ...
    // Cradlepoint categories: WWAN Routers, Switches & APs, WWAN Adapters, Private 5G, Accessories
    const categoryMapping: readonly CategoryMapping[] = [
      { index: 0, section: 'wwan-routers', name: 'WWAN Routers' },
      { index: 1, section: 'switches-access-points', name: 'LAN Switches & Access Points' },
      { index: 2, section: 'wwan-adapters', name: 'WWAN Adapters' },
      { index: 3, section: 'private-5g', name: 'Private 5G / Neutral Host' },
      { index: 4, section: 'wwan-accessories', name: 'WWAN Accessories' }
    ];
  ...
  function initComparisonTable(): void {
    ...
    function highlightColumn(colIndex: number, active: boolean): void {
      allRows.forEach(function (row: HTMLTableRowElement): void {
        ...
        if (cell) {
          if (active) {
            (cell as HTMLElement).style.backgroundColor = 'rgba(0, 125, 195, 0.06)';
          } else {
    ...
    // Cradlepoint comparison table columns map to WWAN Routers and WWAN Adapters sections
    const scrollTargets: readonly ScrollTargetResolver[] = [
      function (): HTMLElement | null { return document.getElementById('wwan-routers'); },
      function (): HTMLElement | null { return document.getElementById('wwan-routers'); },
      function (): HTMLElement | null { return document.querySelector('.featured-products--adapters'); },
      function (): HTMLElement | null { return document.querySelector('.featured-products--adapters'); }
    ];
